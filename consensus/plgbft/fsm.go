...
type fsm struct {
	// PlgBFT consensus protocol configuration
	config *PlgBFTConfig
	...
	// plgbftBackend implements methods needed from the plgbft
...
func (f *fsm) Validate(proposal []byte) error {
	...
	if err := extra.ValidateParentSignatures(block.Number(), f.plgbftBackend, nil, f.parent, parentExtra,
	...
	if f.logger.IsTrace() && block.Number() > 1 {
		validators, err := f.plgbftBackend.GetValidators(block.Number()-2, nil)
		if err != nil {
...
func validateHeaderFields(parent *types.Header, header *types.Header) error {
	...
	// verify mix digest
	if header.MixHash != PlgBFTMixDigest {
