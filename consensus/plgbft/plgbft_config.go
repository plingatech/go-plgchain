...
// PlgBFTConfig is the configuration file for the Plgbft consensus protocol.
type PlgBFTConfig struct {
...
// GetPlgBFTConfig deserializes provided chain config and returns PlgBFTConfig
func GetPlgBFTConfig(chainConfig *chain.Chain) (PlgBFTConfig, error) {
	consensusConfigJSON, err := json.Marshal(chainConfig.Params.Engine["plgbft"])
	if err != nil {
		return PlgBFTConfig{}, err
	}

	var plgBFTConfig PlgBFTConfig
	err = json.Unmarshal(consensusConfigJSON, &plgBFTConfig)

	if err != nil {
		return PlgBFTConfig{}, err
	}

	return plgBFTConfig, nil
}
...
func (p *PlgBFTConfig) IsBridgeEnabled() bool {
	return p.Bridge != nil
}
...
// TokenConfig is the configuration of native token used by plgchain network
