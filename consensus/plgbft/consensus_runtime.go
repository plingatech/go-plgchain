...
import (
	...
	"github.com/plingatech/plg-ibft/messages"
...
func (c *consensusRuntime) initStateSyncManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		stateSenderAddr := c.config.PlgBFTConfig.Bridge.BridgeAddr
		stateSyncManager, err := newStateSyncManager(
			...
			&stateSyncConfig{
				...
				stateSenderStartBlock: c.config.PlgBFTConfig.Bridge.EventTrackerStartBlocks[stateSenderAddr],
				jsonrpcAddr:           c.config.PlgBFTConfig.Bridge.JSONRPCEndpoint,
...
func (c *consensusRuntime) initCheckpointManager(logger hcf.Logger) error {
	if c.IsBridgeEnabled() {
		...
		txRelayer, err := txrelayer.NewTxRelayer(txrelayer.WithIPAddress(c.config.PlgBFTConfig.Bridge.JSONRPCEndpoint))
		if err != nil {
			return err
		}
		...
		c.checkpointManager = newCheckpointManager(
			...
			c.config.PlgBFTConfig.Bridge.CheckpointAddr,
			txRelayer,
			c.config.blockchain,
			c.config.plgbftBackend,
...
func (c *consensusRuntime) IsBridgeEnabled() bool {
	return c.config.PlgBFTConfig.IsBridgeEnabled()
}
...
func (c *consensusRuntime) FSM() error {
	...
	ff := &fsm{
		config:            c.config.PlgBFTConfig,
		parent:            parent,
		backend:           c.config.blockchain,
		plgbftBackend:     c.config.plgbftBackend,
...
func (c *consensusRuntime) restartEpoch(header *types.Header) (*epochMetadata, error) {
	...
	validatorSet, err := c.config.plgbftBackend.GetValidators(header.Number, nil)
	if err != nil {
...
func (c *consensusRuntime) calculateCommitEpochInput(
	...
	if blockHeader.Number > commitEpochLookbackSize {
		for i := 0; i < commitEpochLookbackSize; i++ {
			validators, err := c.config.plgbftBackend.GetValidators(blockHeader.Number-2, nil)
			if err != nil {
				return nil, err
			}
...
func (c *consensusRuntime) isFixedSizeOfEpochMet(blockNumber uint64, epoch *epochMetadata) bool {
	return epoch.FirstBlockInEpoch+c.config.PlgBFTConfig.EpochSize-1 == blockNumber
}
...
func (c *consensusRuntime) isFixedSizeOfSprintMet(blockNumber uint64, epoch *epochMetadata) bool {
	return (blockNumber-epoch.FirstBlockInEpoch+1)%c.config.PlgBFTConfig.SprintSize == 0
}
