// Package plgbft implements PBFT consensus algorithm integration and bridge feature
package plgbft
...
import (
	...
	"github.com/plingatech/go-plgchain/network"
	"github.com/plingatech/go-plgchain/state"
	"github.com/plingatech/go-plgchain/syncer"
...
// plgbftBackend is an interface defining plgbft methods needed by fsm and sync tracker
type plgbftBackend interface {
...
func Factory(params *consensus.Params) (consensus.Consensus, error) {
	logger := params.Logger.Named("plgbft")
	...
	// initialize plgbft consensus config
	customConfigJSON, err := json.Marshal(params.Config.Config)
	if err != nil {
		return nil, err
	}

	err = json.Unmarshal(customConfigJSON, &plgbft.consensusConfig)
	if err != nil {
		return nil, err
	}
...
type Plgbft struct {
	...
	// consensusConfig is genesis configuration for plgbft consensus protocol
...
func GenesisPostHookFactory(config *chain.Chain, engineName string) func(txn *state.Transition) error {
	return func(transition *state.Transition) error {
		plgBFTConfig, err := GetPlgBFTConfig(config)
		if err != nil {
			return err
		}

		// initialize ChildValidatorSet SC
		input, err := getInitChildValidatorSetInput(plgBFTConfig)
		if err != nil {
			return err
		}
		...
		input, err = getInitChildERC20PredicateInput(plgBFTConfig.Bridge)
		if err != nil {
			return err
		}
		...
		if plgBFTConfig.Bridge != nil {
			rootNativeERC20Token = plgBFTConfig.Bridge.RootNativeERC20Addr
		}

		if plgBFTConfig.MintableNativeToken {
			...
			params := &contractsapi.InitializeNativeERC20MintableFn{
				...
				Owner_:     plgBFTConfig.Governance,
				RootToken_: rootNativeERC20Token,
				Name_:      plgBFTConfig.NativeTokenConfig.Name,
				Symbol_:    plgBFTConfig.NativeTokenConfig.Symbol,
				Decimals_:  plgBFTConfig.NativeTokenConfig.Decimals,
			}
...
func (p *Plgbft) Initialize() error {
	p.logger.Info("initializing plgbft...")
	...
	// initialize plgbft consensus data directory
	p.dataDir = filepath.Join(p.config.Config.Path, "plgbft")
...
func (p *Plgbft) Start() error {
	p.logger.Info("starting plgbft consensus", "signer", p.key.String())
...
func (p *Plgbft) initRuntime() error {
	runtimeConfig := &runtimeConfig{
		PlgBFTConfig:          p.consensusConfig,
		Key:                   p.key,
		DataDir:               p.dataDir,
		State:                 p.state,
...
func (p *Plgbft) startRuntime() error {
	go p.startConsensusProtocol()

	return nil
}

func (p *Plgbft) startConsensusProtocol() {
...
func (p *Plgbft) waitForNPeers() bool {
	for {
		select {
		case <-p.closeCh:
			return false
...
func (p *Plgbft) GetSyncProgression() *progress.Progression {
...
func (p *Plgbft) VerifyHeader(header *types.Header) error {
...
func (p *Plgbft) verifyHeaderImpl(parent, header *types.Header, parents []*types.Header) error {
...
func (p *Plgbft) GetValidators(blockNumber uint64, parents []*types.Header) (AccountSet, error) {
...
func (p *Plgbft) ProcessHeaders(_ []*types.Header) error {
	// Not required
	return nil
}

// GetBlockCreator retrieves the block creator (or signer) given the block header
func (p *Plgbft) GetBlockCreator(h *types.Header) (types.Address, error) {
	return types.BytesToAddress(h.Miner), nil
}

// PreCommitState a hook to be called before finalizing state transition on inserting block
func (p *Plgbft) PreCommitState(_ *types.Header, _ *state.Transition) error {
	// Not required
	return nil
}

// GetBridgeProvider returns an instance of BridgeDataProvider
func (p *Plgbft) GetBridgeProvider() consensus.BridgeDataProvider {
	return p.runtime
}
