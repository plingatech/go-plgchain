...
import (
	...
	secretsHelper "github.com/plingatech/go-plgchain/secrets/helper"
...
func TestIntegration_CommitEpoch(t *testing.T) {
	...
	for _, currentValidators := range validatorSets {
		...
		// validator data for plgbft config
		...
		for i, validator := range accSet {
			...
			// create validator data for plgbft config
		...
		initInput, err := getInitChildValidatorSetInput(plgBFTConfig)
		require.NoError(t, err)
		...
		commitEpoch := createTestCommitEpochInput(t, 1, accSet, plgBFTConfig.EpochSize)
		...
		commitEpoch = createTestCommitEpochInput(t, 2, accSet, plgBFTConfig.EpochSize)
		input, err = commitEpoch.EncodeAbi()
		require.NoError(t, err)
