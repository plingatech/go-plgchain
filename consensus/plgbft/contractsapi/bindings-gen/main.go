...
import (
	...
	gensc "github.com/plingatech/go-plgchain/consensus/plgbft/contractsapi"
...
`
	...
	if err := ioutil.WriteFile("./consensus/plgbft/contractsapi/contractsapi.go", output, 0600); err != nil {
		log.Fatal(err)
	}
}
...
func generateType(generatedData *generatedData, name string, obj *abi.Type, res *[]string) (string, error) {
	...
	for _, tupleElem := range obj.TupleElems() {
		...
		// uint8 internally in plgbft, we can use regexp to replace those values with the
