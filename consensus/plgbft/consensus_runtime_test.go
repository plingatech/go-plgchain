...
func TestConsensusRuntime_isFixedSizeOfEpochMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.PlgBFTConfig.EpochSize = c.epochSize
...
func TestConsensusRuntime_isFixedSizeOfSprintMet_NotReachedEnd(t *testing.T) {
	...
	for _, c := range cases {
		runtime.config.PlgBFTConfig.SprintSize = c.sprintSize
		runtime.epoch.FirstBlockInEpoch = c.firstBlockInEpoch
		assert.False(t,
...
func TestConsensusRuntime_OnBlockInserted_EndOfEpoch(t *testing.T) {
	...
	plgbftBackendMock := new(plgbftBackendMock)
	plgbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validatorSet).Times(3)
	...
	config := &runtimeConfig{
		...
		plgbftBackend: plgbftBackendMock,
		txPool:        txPool,
...
func TestConsensusRuntime_OnBlockInserted_MiddleOfEpoch(t *testing.T) {
	...
	plgbftBackendMock := new(plgbftBackendMock)
	plgbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(nil).Once()
	...
	config := &runtimeConfig{
		PlgBFTConfig: &PlgBFTConfig{EpochSize: epochSize},
		blockchain:   blockchainMock,
		txPool:       txPool,
	}
...
func Test_NewConsensusRuntime(t *testing.T) {
	...
	plgbftBackendMock := new(plgbftBackendMock)
	plgbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators).Twice()
	...
	assert.Equal(t, uint64(10), runtime.config.PlgBFTConfig.SprintSize)
	assert.Equal(t, uint64(10), runtime.config.PlgBFTConfig.EpochSize)
	assert.Equal(t, "0x0000000000000000000000000000000000000101", contracts.ValidatorSetContract.String())
	assert.Equal(t, "0x1300000000000000000000000000000000000000", runtime.config.PlgBFTConfig.Bridge.BridgeAddr.String())
	assert.Equal(t, "0x1000000000000000000000000000000000000000", runtime.config.PlgBFTConfig.Bridge.CheckpointAddr.String())
	assert.True(t, runtime.IsBridgeEnabled())
	systemStateMock.AssertExpectations(t)
	blockchainMock.AssertExpectations(t)
	plgbftBackendMock.AssertExpectations(t)
}
...
func TestConsensusRuntime_calculateCommitEpochInput_SecondEpoch(t *testing.T) {
	...
	plgbftBackendMock := new(plgbftBackendMock)
	plgbftBackendMock.On("GetValidators", mock.Anything, mock.Anything).Return(validators.getPublicIdentities()).Twice()
...
func TestConsensusRuntime_HasQuorum(t *testing.T) {
	...
	config := &runtimeConfig{
		...
		blockchain:   blockchainMock,
		PlgBFTConfig: &PlgBFTConfig{EpochSize: 10, SprintSize: 5},
	}
	runtime := &consensusRuntime{
		state:          state,
		config:         config,
