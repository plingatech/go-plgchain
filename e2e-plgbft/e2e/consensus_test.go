...
func TestE2E_Consensus_RegisterValidator(t *testing.T) {
	...
	systemState := plgbft.NewSystemState(
	...
	extra, err := plgbft.GetIbftExtra(genesisBlock.ExtraData)
	require.NoError(t, err)
...
func TestE2E_Consensus_Validator_Unstake(t *testing.T) {
	...
	rootchainValidators := []*plgbft.ValidatorInfo{}
