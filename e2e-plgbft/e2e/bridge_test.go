...
import (
	...
	rootchainHelper "github.com/plingatech/go-plgchain/command/rootchain/helper"
...
func TestE2E_Bridge_DepositAndWithdrawERC20(t *testing.T) {
	...
	manifest, err := plgbft.LoadManifest(path.Join(cluster.Config.TmpDir, manifestFileName))
	require.NoError(t, err)
	...
	currentExtra, err := plgbft.GetIbftExtra(currentBlock.ExtraData)
	require.NoError(t, err)
...
func TestE2E_Bridge_ChangeVotingPower(t *testing.T) {
	...
	queryValidators := func(handler func(idx int, validatorInfo *plgbft.ValidatorInfo)) {
	...
	originalValidatorStorage := make(map[ethgo.Address]*plgbft.ValidatorInfo, votingPowerChanges)

	queryValidators(func(idx int, validator *plgbft.ValidatorInfo) {
