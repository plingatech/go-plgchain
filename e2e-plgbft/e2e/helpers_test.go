...
func getRootchainValidators(relayer txrelayer.TxRelayer, checkpointManagerAddr ethgo.Address) ([]*plgbft.ValidatorInfo, error) {
	...
	validators := make([]*plgbft.ValidatorInfo, validatorsCount)
	...
	for i := 0; i < int(validatorsCount); i++ {
		...
		validators[i] = &plgbft.ValidatorInfo{
...
func sendExitTransaction(
	...
	proofExitEventEncoded, err := exitEventAPI.Encode(&plgbft.ExitEvent{
		ID:       exitEventID,
