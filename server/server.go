...
import (
	...
	"github.com/plingatech/go-plgchain/blockchain/storage"
	"github.com/plingatech/go-plgchain/blockchain/storage/leveldb"
	"github.com/plingatech/go-plgchain/blockchain/storage/memory"
	consensusPlgBFT "github.com/plingatech/go-plgchain/consensus/plgbft"

	"github.com/hashicorp/go-hclog"
	"github.com/plingatech/go-plgchain/archive"
	"github.com/plingatech/go-plgchain/blockchain"
	"github.com/plingatech/go-plgchain/chain"
	"github.com/plingatech/go-plgchain/consensus"
	"github.com/plingatech/go-plgchain/consensus/plgbft/statesyncrelayer"
	"github.com/plingatech/go-plgchain/consensus/plgbft/wallet"
	"github.com/plingatech/go-plgchain/contracts"
	"github.com/plingatech/go-plgchain/crypto"
	"github.com/plingatech/go-plgchain/helper/common"
	configHelper "github.com/plingatech/go-plgchain/helper/config"
	"github.com/plingatech/go-plgchain/helper/progress"
	"github.com/plingatech/go-plgchain/jsonrpc"
	...
	"github.com/plingatech/go-plgchain/state/runtime"
	"github.com/plingatech/go-plgchain/state/runtime/allowlist"
	"github.com/plingatech/go-plgchain/state/runtime/tracer"
	"github.com/plingatech/go-plgchain/txpool"
	"github.com/plingatech/go-plgchain/types"
	"github.com/plingatech/go-plgchain/validate"
...
type Server struct {
	...
	// stateSyncRelayer is handling state syncs execution (Plgbft exclusive)
...
func NewServer(config *Config) (*Server, error) {
	...
	if ConsensusType(engineName) == PlgBFTConsensus {
		plgBFTConfig, err := consensusPlgBFT.GetPlgBFTConfig(config.Chain)
		if err != nil {
			return nil, err
		}

		if plgBFTConfig.InitialTrieRoot != types.ZeroHash {
			checkedInitialTrieRoot, err := itrie.HashChecker(plgBFTConfig.InitialTrieRoot.Bytes(), stateStorage)
			if err != nil {
				return nil, fmt.Errorf("error on state root verification %w", err)
			}

			if checkedInitialTrieRoot != plgBFTConfig.InitialTrieRoot {
				return nil, errors.New("invalid initial state root")
			}

			logger.Info("Initial state root checked and correct")

			initialStateRoot = plgBFTConfig.InitialTrieRoot
		}
	}
...
func (s *Server) setupRelayer() error {
	...
	plgBFTConfig, err := consensusPlgBFT.GetPlgBFTConfig(s.config.Chain)
	if err != nil {
		return fmt.Errorf("failed to extract plgbft config: %w", err)
	}

	trackerStartBlockConfig := map[types.Address]uint64{}
	if plgBFTConfig.Bridge != nil {
		trackerStartBlockConfig = plgBFTConfig.Bridge.EventTrackerStartBlocks
	}
