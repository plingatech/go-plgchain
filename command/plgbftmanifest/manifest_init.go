...
func GetCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "manifest",
		Short:   "Initializes manifest file. It is applicable only to plgbft consensus protocol.",
		PreRunE: runPreRun,
		Run:     runCommand,
	}
...
func setFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(
		...
		validatorsPathFlag,
		"./",
		"root path containing plgbft validator keys",
	)
	...
	cmd.Flags().StringVar(
		...
		"folder prefix names for plgbft validator keys",
	)
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	manifest := &plgbft.Manifest{GenesisValidators: validators, ChainID: params.chainID}
...
func (p *manifestInitParams) getValidatorAccounts() ([]*plgbft.Validator, error) {
	...
	if len(p.validators) > 0 {
		validators := make([]*plgbft.Validator, len(p.validators))
