...
func (p *genesisParams) validateFlags() error {
	...
	if p.epochSize < 2 && (p.isIBFTConsensus() || p.isPlgBFTConsensus()) {
...
func (p *genesisParams) isPlgBFTConsensus() bool {
	return server.ConsensusType(p.consensusRaw) == server.PlgBFTConsensus
}
...
func (p *genesisParams) initRawParams() error {
	...
	if p.consensus == server.PlgBFTConsensus {
		return nil
	}
