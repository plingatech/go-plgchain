...
import (
	...
	"github.com/plingatech/go-plgchain/secrets/helper"
)

// common flags for all plgbft commands
const (
	AccountDirFlag    = "data-dir"
	AccountConfigFlag = "config"

	AccountDirFlagDesc    = "the directory for the Go Plgchain data if the local FS is used"
	AccountConfigFlagDesc = "the path to the SecretsManager config file, if omitted, the local FS secrets manager is used"
)

// common errors for all plgbft commands
var (
