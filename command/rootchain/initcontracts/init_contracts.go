...
var (
	...
	metadataPopulatorMap = map[string]func(*plgbft.RootchainConfig, types.Address){
		stateSenderName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.StateSenderAddress = addr
		},
		checkpointManagerName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.CheckpointManagerAddress = addr
		},
		blsName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.BLSAddress = addr
		},
		bn256G2Name: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.BN256G2Address = addr
		},
		exitHelperName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.ExitHelperAddress = addr
		},
		rootERC20PredicateName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootERC20PredicateAddress = addr
		},
		rootERC20Name: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootNativeERC20Address = addr
		},
		erc20TemplateName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.ERC20TemplateAddress = addr
		},
		rootERC721PredicateName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootERC721PredicateAddress = addr
		},
		rootERC721Name: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootERC721Address = addr
		},
		rootERC1155PredicateName: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
			rootchainConfig.RootERC1155PredicateAddress = addr
		},
		rootERC1155Name: func(rootchainConfig *plgbft.RootchainConfig, addr types.Address) {
...
func runCommand(cmd *cobra.Command, _ []string) {
	...
	manifest, err := plgbft.LoadManifest(params.manifestPath)
	if err != nil {
...
func deployContracts(outputter command.OutputFormatter, client *jsonrpc.Client,
	manifest *plgbft.Manifest) error {
	...
	rootchainConfig := &plgbft.RootchainConfig{}
...
func populateExistingTokenAddr(eth *jsonrpc.Eth, tokenAddr, tokenName string, manifest *plgbft.Manifest) error {
...
func initializeExitHelper(txRelayer txrelayer.TxRelayer, rootchainConfig *plgbft.RootchainConfig,
	deployerKey ethgo.Key) error {
...
func initializeRootERC20Predicate(txRelayer txrelayer.TxRelayer, rootchainConfig *plgbft.RootchainConfig,
	deployerKey ethgo.Key) error {
...
func validatorSetToABISlice(o command.OutputFormatter,
	validators []*plgbft.Validator) ([]*contractsapi.Validator, error) {
	accSet := make(plgbft.AccountSet, len(validators))
	...
	for i, validator := range validators {
		...
		accSet[i] = &plgbft.ValidatorMetadata{
