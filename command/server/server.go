...
import (
	...
	"github.com/plingatech/go-plgchain/command/server/config"
	"github.com/plingatech/go-plgchain/command/server/export"
...
func GetCommand() *cobra.Command {
	serverCmd := &cobra.Command{
		Use:     "server",
		Short:   "The default command that starts the Go Plgchain client, by bootstrapping all modules together",
		PreRunE: runPreRun,
		Run:     runCommand,
	}
...
func setFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(
		...
		"the data directory used for storing Go Plgchain client data",
	)
	...
	cmd.Flags().BoolVar(
		...
		"start the state sync relayer service (PlgBFT only)",
	)
